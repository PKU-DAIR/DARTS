...
class ScoreFilter:
    ...
    def update(self, id2mean):
        for k, mean_val in id2mean.items():
            if self.counts[k] == 0:
                self.scores[k] = mean_val
            else:
                self.scores[k] = self.alpha * mean_val + (1 - self.alpha) * self.scores[k]
            self.counts[k] += 1
...
class RayPPOTrainer:
    ...
    def __init__(
        ...
        """
        self.yita = config.algorithm.get("yita", 1.0)
    ...
    def _create_dataloader(self, train_dataset, val_dataset, collate_fn, train_sampler: Optional[Sampler]):
        ...
        self.train_dataloader = StatefulDataLoader(
            ...
            batch_size=int(self.config.data.get("gen_batch_size", self.config.data.train_batch_size) * self.yita),
        ...
        for epoch in range(self.config.trainer.total_epochs):
            ...
            for batch_dict in self.train_dataloader:
                ...
                with marked_timer("step", timing_raw):
                    ...
                    resp_len_llm = resp_mask.sum(dim=1)  # 响应长度
                    s = batch.batch["token_level_scores"].sum(-1)  # 总奖励
                    ...
                    with torch.no_grad():
                        for k, s_val in zip(keys, scores_list):
                            acc[k] += s_val
                            cnt[k] += 1
                    ...
                    # 新增：更新长度过滤器
                    acc_len, cnt_len = defaultdict(float), defaultdict(int)
                    for k, l_val in zip(keys, resp_len_llm_list):
                        acc_len[k] += float(l_val)
                        cnt_len[k] += 1
                    id2mean_len = {k: acc_len[k] / cnt_len[k] for k in acc_len}
                ...
                with open(out_path, "a", encoding="utf-8", newline="") as f:
                    writer = csv.writer(f)
                    if not file_exists:
                        writer.writerow(["epoch", "key", "mean_reward", "response_length"])
                    for k in sorted(all_keys):
                        mr = scores_epoch.get(k)
                        rl = len_epoch.get(k)
                        writer.writerow([
                            epoch,
                            k,
                ...
                with open(events_path, "a", encoding="utf-8", newline="") as f:
                    writer = csv.writer(f)
                    if not file_exists:
                        writer.writerow(["epoch", "key", "occurrence", "reward", "response_length"])
