...
class DataProto:
    ...
    def custom_repeat(self, repeat_times):
        """
        Repeat each sample in the batch a different number of times.
        
        Args:
            repeat_times (list or torch.Tensor): Number of times to repeat each sample.
                Length should match the batch size.
        ...
        if self.batch is not None:
            repeated_tensors = {}
            for key, tensor in self.batch.items():
                # Each tensor should be repeated according to repeat_times for each sample
