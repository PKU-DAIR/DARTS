...
def rearrange_micro_batches(
    ...
    if kv_cache_lengths is not None:
        # 将 kv_cache_lengths 转换为 tensor（如果还不是）
        if not isinstance(kv_cache_lengths, torch.Tensor):
            kv_cache_lengths = torch.tensor(kv_cache_lengths, dtype=seq_len_effective.dtype, device=seq_len_effective.device)
        seq_len_effective = seq_len_effective + kv_cache_lengths
...
def prepare_dynamic_batch(data: DataProto, max_token_len: int) -> tuple[list[DataProto], list[list[int]]]:
    ...
    """
    kv_cache_lengths = None
    if "kv_cache_lengths" in data.non_tensor_batch:
        kv_cache_lengths = data.non_tensor_batch["kv_cache_lengths"]
        # 从 non_tensor_batch 中删除 kv_cache_lengths，避免后续索引问题
        del data.non_tensor_batch["kv_cache_lengths"]
    batch, batch_idx_list = rearrange_micro_batches(data.batch, max_token_len=max_token_len, kv_cache_lengths=kv_cache_lengths)
    micro_batches = []
