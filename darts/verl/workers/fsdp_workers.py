...
class ActorRolloutRefWorker(Worker, DistProfilerExtension):
    ...
    def update_actor(self, data: DataProto):
        ...
        with self.ulysses_sharding_manager:
            ...
            with Timer(name="update_policy", logger=None) as timer:
                #self.actor.last_chunks = is_last
    ...
    @register(dispatch_mode=Dispatch.DP_COMPUTE_PROTO)
    @DistProfiler.annotate(color="blue", role="update")
    def update(self):
        self.actor._optimizer_step()
        self.actor.actor_optimizer.zero_grad()
        self.actor_lr_scheduler.step()
        result = DataProto(meta_info={"metrics": {"actor/lr": self.actor_lr_scheduler.get_last_lr()[0]}})
        return result

    @register(dispatch_mode=Dispatch.DP_COMPUTE_PROTO)
    @DistProfiler.annotate(color="blue", role="clear")
    def clear_kv_cache(self):
        self.actor.clear_kv_cache()
        result = DataProto(meta_info={"metrics": {"actor/lr":1}})
        return result
    
    @register(dispatch_mode=Dispatch.DP_COMPUTE_PROTO)
    @DistProfiler.annotate(color="blue", role="clear")
    def clear_kv_cache2(self):
        self.ref_policy.clear_kv_cache()
        result = DataProto(meta_info={"metrics": {"actor/lr": 1}})
        return result
