...
class VarienceFilter:
    ...
    def get_varience(self, key):
        """获取某个ID的当前分数"""
        if key in self.varience:
            return self.varience[key]
        
        if len(self.varience) > 0:
            return sum(self.varience.values()) / len(self.varience)
...
class RayTrainerStream3(OneStepOffRayTrainer):
    ...
    def _submit_old_logprob_compute(self, batch: DataProto) -> DataProto:
        ...
        if "entropys" in old_log_prob.batch:
            batch.batch['entropys'] = old_log_prob.batch.pop("entropys")
                ...
                async for sample_idx, partial_result in self.async_rollout_manager.async_generate_sequences_token_stream(gen_batch, chunk_size=chunk_size):
                    ...
                    if flag and ((len(finished_samples) % (global_n//4)) == 0):
                        ...
                        if batch_for_compute is not None:
                            ...
                            single_sample_protos = computed_batch.split(split_size=1)

                            for i in range(len(single_sample_protos)):
                                sample_idx = sample_idx_list[i]
                                results[sample_idx] = single_sample_protos[i]
        ...
        for epoch in range(self.config.trainer.total_epochs):
            ...
            for batch_dict in self.train_dataloader:
                ...
                if False:
                    ...
                    cur_variance = []
                    avg_r = 10 # min(max((1+ 1.5*math.log(5 * (self._var_step_avg) / avg_token_len) )* self.config.actor_rollout_ref.rollout.n , 8), 14)
                    ...
                    total_m = (self.config.data.train_batch_size *avg_r // 8) * 8
                    optimizer = ResourceOptimizer(cur_variance, total_m, min_m=10, max_m=10)
                    ...
                    result = [min((16 - x)//2, 1) for x in cur_redundancy]
                    print(f"当前avg_token_len: {avg_token_len}")
                    print(f"当前varience_ema: {round(self._var_step_avg,2)}")
                    print(f"avg_r: {round(avg_r,2)}")
                ...
                with marked_timer("step", timing_raw):
                    ...
                    self._var_step_q.append(varience_step / len(resp_len_std_by_key))
