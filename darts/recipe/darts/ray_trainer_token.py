...
class RayTrainerStream3(OneStepOffRayTrainer):
    ...
    async def _generate_with_token_streaming_overlapped(self, gen_batch: DataProto, chunk_size: int = 100) -> DataProto:
        """
        使用token级流式生成,并重叠计算old_log_prob和ref_log_prob
        
        策略: 在流式生成进行时就启动log_prob计算的异步任务，
        但计算使用的是当前的partial_result（包含到目前为止的所有tokens）
        ...
        print(f"开始token级流式生成+重叠计算,batch_size={len(gen_batch)}, chunk_size={chunk_size}")
        ...
        if hasattr(self.async_rollout_manager, 'async_generate_sequences_token_stream'):
            ...
            accumulate_tokens = 0
            batch = None
            print(f"等待 {expected_samples} 个样本的流式生成+重叠计算...")
            ...
            try:
                ...
                async for sample_idx, partial_result in self.async_rollout_manager.async_generate_sequences_token_stream(gen_batch, chunk_size=chunk_size):
                    ...
                    if sample_idx in sample_accumulators:
                        accumulate_tokens += int(torch.sum(partial_result.batch['response_mask'][0]).item()) - int(torch.sum(sample_accumulators[sample_idx].batch['response_mask'][0]).item())
                    else:
                        accumulate_tokens += int(torch.sum(partial_result.batch['response_mask'][0]).item())

                    # 检查实际生成的token数量（通过response_mask统计非零元素）
                    ...
                    if sample_idx == 0:
                        if 'response_mask' in partial_result.batch:
                            # response_mask中1表示有效token，0表示padding
                            actual_response_tokens = int(torch.sum(partial_result.batch['response_mask'][0]).item())
                        print(f" 🚀样本0 {actual_response_tokens}")
                    
                    
                    if accumulate_tokens >= 100 * 1024 and len(sample_accumulators) == 1024:
                        accumulate_tokens = 0  
                ...
                batch = DataProto.concat(final_results)
                batch = self._submit_old_logprob_compute(batch)
                batch = self._submit_ref_logprob_compute(batch)   
                ...
                # 如果之前已经进行了重叠计算，返回计算过的batch
                # 否则返回完整的final_result（但没有计算log_prob）
                if batch is not None:
                    ...
                    print("⚠️ 返回未计算log_prob的结果")
                    return final_result
                    
            except Exception as e:
                print(f"❌ 流式生成+重叠计算失败: {e}")
                print("回退到标准生成方法...")
        else:
            print("❌ 流式生成不可用，回退到标准生成方法")
        
        # 如果所有流式生成都失败，返回None表示需要使用标准生成
        return None
        
        
    
    def _submit_old_logprob_compute(self, batch: DataProto) -> DataProto:
        """提交old_log_prob计算（同步版本）"""
    ...
    def _submit_ref_logprob_compute(self, batch: DataProto) -> DataProto:
        """提交ref_log_prob计算（同步版本）"""
        ref_log_prob = self.ref_policy_wg.compute_ref_log_prob(batch)
        batch = batch.union(ref_log_prob)
        return batch
        
    async def _collect_old_logprob_results(self, task_dict: Dict[int, str]) -> Dict[int, DataProto]:
        """收集old_log_prob计算结果"""
        results = {}
        ...
        try:
            ...
            for sample_idx, task_id in task_dict.items():
                worker = self.actor_rollout_wg.workers[0]
                if hasattr(worker, 'get_streaming_compute_result'):
                    task = worker.get_streaming_compute_result.remote(task_id)
                    tasks.append(task)
            ...
            if tasks:
                ...
                for sample_idx, result in zip(sample_indices, completed_results):
                    if not isinstance(result, Exception) and result is not None:
                        results[sample_idx] = result
                    else:
                        print(f"⚠️ 样本 {sample_idx} old_log_prob计算失败: {result}")
            
        except asyncio.TimeoutError:
            print("⚠️ old_log_prob计算超时")
        except Exception as e:
            print(f"❌ 收集old_log_prob结果失败: {e}")
        
        return results
    
    async def _collect_ref_logprob_results(self, task_dict: Dict[int, str]) -> Dict[int, DataProto]:
        """收集ref_log_prob计算结果"""
        results = {}
        
        try:
            if not hasattr(self, 'ref_policy_wg') or not self.ref_policy_wg:
                return results
            ...
            for sample_idx, task_id in task_dict.items():
                worker = self.ref_policy_wg.workers[0]
                if hasattr(worker, 'get_streaming_compute_result'):
                    task = worker.get_streaming_compute_result.remote(task_id)
                    tasks.append(task)
                    sample_indices.append(sample_idx)
            ...
            if tasks:
                ...
                for sample_idx, result in zip(sample_indices, completed_results):
                    if not isinstance(result, Exception) and result is not None:
                        ...
                        print(f"⚠️ 样本 {sample_idx} ref_log_prob计算失败: {result}")
            
        except asyncio.TimeoutError:
            print("⚠️ ref_log_prob计算超时")
        except Exception as e:
            print(f"❌ 收集ref_log_prob结果失败: {e}")
            ...
            for batch_dict in self.train_dataloader:
                ...
                with marked_timer("step", timing_raw):
                    ...
                    with marked_timer("gen", timing_raw, color="red"):
                        ...
                        enable_overlapped_compute = self.config.trainer.get("enable_overlapped_compute", True)
                        
                        if use_token_streaming and enable_overlapped_compute:
                            print(f"🚀 使用token级流式生成+重叠计算 (chunk_size={chunk_size})")
                            import asyncio
                            loop = asyncio.new_event_loop()
                            asyncio.set_event_loop(loop)
                            try:
                                gen_batch_output = loop.run_until_complete(
                                    self._generate_with_token_streaming_overlapped(gen_batch, chunk_size=chunk_size)
                                )
                                ...
                                loop.close()      
                        elif use_token_streaming:
                            print(f"使用token级流式生成 (chunk_size={chunk_size})")
                            # 使用异步运行流式生成
                            import asyncio
                            loop = asyncio.new_event_loop()
                            asyncio.set_event_loop(loop)
                            try:
                                gen_batch_output = loop.run_until_complete(
                                    self._generate_with_token_streaming(gen_batch, chunk_size=chunk_size)
                                )
                                has_old_logprob = False
                                has_ref_logprob = False
                            finally:
