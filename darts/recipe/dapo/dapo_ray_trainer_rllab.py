...
class RayDAPOTrainerRLLab(RayPPOTrainer):
    ...
    def rollout(self, id):
        ...
        ref  = self.async_rollout_manager.async_generate_sequences_rllab(gen_batch,id)
        self.wait_rollout_list[ref] = (id, new_batch)
            ...
            while self.global_steps <= self.total_training_steps:
                ...
                with marked_timer("step", timing_raw):
                    for i in range(self.workers_num):
                        self.rollout(i)
                    while num_valid_prompts_collected < self.config.data.train_batch_size:
                        # Wait for at least one task to complete
                        if not self.wait_rollout_list:
                            # Should not happen if workers_num > 0 and we pre-dispatched
                            print("Warning: No pending rollout tasks!")
                            break
                            
                        ready_refs, remaining_refs = ray.wait(list(self.wait_rollout_list.keys()), num_returns=1)
                        
                        for ref in ready_refs:
                            # 1. Get result
                            gen_batch_output = ray.get(ref)
                            worker_id, input_batch = self.wait_rollout_list.pop(ref)
                            ...
                            if batch_chunk is not None and len(batch_chunk) > 0:
                                collected_batch = DataProto.concat([collected_batch, batch_chunk]) if collected_batch.batch else batch_chunk
                                num_valid_prompts_collected += valid_count
                                print(f"Collected {num_valid_prompts_collected}/{self.config.data.train_batch_size} valid prompts so far.")
                                
                            # Check max generation batches to avoid infinite loop
                            ...
                            if max_num_gen_batches > 0 and num_gen_batches >= max_num_gen_batches:
                                if num_valid_prompts_collected < self.config.data.train_batch_size:
                                    ...
                                    # Break inner loop to proceed with whatever we have or skip step
                                    break
                        
                        if max_num_gen_batches > 0 and num_gen_batches >= max_num_gen_batches:
                            break

                    # 🔧 Critical: Wait for all remaining rollout tasks to complete
                    # This prevents memory leaks and ensures clean state before training
                    if self.wait_rollout_list:
                        print(f"Waiting for {len(self.wait_rollout_list)} remaining rollout tasks to complete...")
                        remaining_refs = list(self.wait_rollout_list.keys())
                        
                        # Wait for all remaining tasks
                        remaining_results = ray.get(remaining_refs)
                        
                        # Process remaining results (optional: add to batch if needed)
                        for ref, result in zip(remaining_refs, remaining_results):
                            worker_id, input_batch = self.wait_rollout_list.pop(ref)
                            # Optionally process these results if we need more data
                            # For now, we just discard them to free memory
                            print(f"Discarding result from worker {worker_id} (already collected enough samples)")
                        
                        print("All rollout tasks completed. Proceeding to training.")
