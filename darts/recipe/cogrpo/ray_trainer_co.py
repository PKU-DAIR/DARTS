...
class CoRayTrainer(OneStepOffRayTrainer):
    ...
    def _prepare_mixed_prompt_batch(self, gen_batch, n):
        ...
        for i, raw in enumerate(expanded_raw_prompts):
            ...
            if isinstance(raw, list):
                msgs = [{"role": "system", "content": sys_prompt}] + raw
            else:
                msgs = [{"role": "system", "content": sys_prompt}, {"role": "user", "content": str(content)}]
        ...
        prompt_texts = [self.tokenizer.apply_chat_template(m, tokenize=False, add_generation_prompt=True) for m in msgs_list]
