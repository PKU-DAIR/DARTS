...
from verl.utils.model import (convert_weight_keys, get_generation_config,
                              update_model_config)
from verl.utils.vllm_utils import patch_vllm_moe_model_weight_loader
from verl.workers.fsdp_workers import ActorRolloutRefWorker as ARRWorker
from verl.workers.fsdp_workers import AsyncActorRolloutRefWorker as AARRWorker
...
class AsyncActorRolloutRefWorker(AARRWorker):
    @register(dispatch_mode=Dispatch.ONE_TO_ALL, blocking=False)
    def sync_rollout_weights(self):
        print("🚀 Start sync weights.")
        assert (self._is_actor or self._is_rollout) and not self.config.hybrid_engine
        assert hasattr(self, "_weights_info") and self._weights_info is not None
...
class RolloutWorker(AsyncActorRolloutRefWorker):
